...
int Bootstrap::run() {
    ...
    Log::message("root", "Starting Bootstrap node DHT at port " + boot_port);

    printCommands();

    processInput();

    Log::message("root", "Exiting VIN Bootstrap node at " + boot_address + ":" + boot_port);
...
int Bootstrap::handleCommands(std::stringstream& command_line) {
    ...
    try {
        ...
        Log::message("root", "Received command: " + command);
