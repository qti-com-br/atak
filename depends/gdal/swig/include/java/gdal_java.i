...
void CPL_STDCALL CPLAndroidErrorHandler(CPLErr eErrClass, int nError, const char * pszErrorMsg)
{
    android_LogPriority p = ANDROID_LOG_UNKNOWN;
    switch(eErrClass) {
        case CE_None :
            p = ANDROID_LOG_INFO;
            break;
        ...
        case CE_Fatal :
            ...
            break;
        default :
            p = ANDROID_LOG_UNKNOWN;
            break;
    }
    __android_log_print(p, "libgdaljni", "%s", pszErrorMsg);
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    CPLSetErrorHandler(CPLAndroidErrorHandler);
