...
%typemap(out) char **dict
{
  ...
  const jclass stringcls = jenv->FindClass("java/lang/String");
  const jmethodID stringconst = jenv->GetMethodID(stringcls, "<init>", "([BLjava/lang/String;)V");
  const jstring charset = jenv->NewStringUTF("ISO-8859-1");
  ...
  if ( stringarray != NULL ) {
    while (*stringarray != NULL ) {
      ...
      if ( pszSep != NULL) {
        ...
        size_t len = strlen(valptr);
        jbyteArray bytes = jenv->NewByteArray(len);
        jenv->SetByteArrayRegion(bytes, 0, len, (jbyte*)valptr);
        jobject value = jenv->NewObject(stringcls, stringconst, bytes, charset);
